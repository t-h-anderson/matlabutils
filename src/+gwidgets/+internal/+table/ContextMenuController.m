classdef ContextMenuController < gwidgets.internal.table.TableController
    ...
    methods (Access = private)
        ...
        function callbacks = callbacks(this)
            owner = this.owner();
            callbacks = struct( ...
                "Group", @(~,e)this.invokeCommand("Group", ...
                    @()owner.Group.requestGroupBy(e.InteractionInformation.DisplayColumn), ...
                    Payload=this.contextPayload(e)), ...
                "AddGroup", @(~,e)this.invokeCommand("AddGroup", ...
                    @()owner.Group.requestAddGroupBy(e.InteractionInformation.DisplayColumn), ...
                    Payload=this.contextPayload(e)), ...
                "SetGroups", @(groupingVariables)this.invokeCommand("SetGroups", ...
                    @()owner.Group.replaceBy(groupingVariables), Value=groupingVariables), ...
                "AddGroups", @(groupingVariables)this.invokeCommand("AddGroups", ...
                    @()owner.Group.addBy(groupingVariables), Value=groupingVariables), ...
                "RemoveGroup", @(groupingVariable)this.invokeCommand("RemoveGroup", ...
                    @()owner.Group.requestRemoveGroupBy(groupingVariable), Value=groupingVariable), ...
                "RemoveGroups", @(groupingVariables)this.invokeCommand("RemoveGroups", ...
                    @()owner.Group.removeBy(groupingVariables), Value=groupingVariables), ...
                "Ungroup", @(~,~)this.invokeCommand("Ungroup", @()owner.Group.requestUngroup()), ...
                "ToggleGroupingMode", @(~,~)this.invokeCommand("ToggleGroupingMode", ...
                    @()owner.Group.requestToggleMode()), ...
                "ToggleShowEmptyGroups", @(~,~)this.invokeCommand("ToggleShowEmptyGroups", ...
                    @()owner.Group.requestToggleShowEmpty()), ...
                "SortAscend", @(~,e)this.invokeCommand("SortAscend", ...
                    @()owner.Sort.requestSortByContext( ...
                    e.InteractionInformation.DisplayRow, e.InteractionInformation.DisplayColumn, "Ascend"), ...
                    Payload=this.contextPayload(e)), ...
                "SortDescend", @(~,e)this.invokeCommand("SortDescend", ...
                    @()owner.Sort.requestSortByContext( ...
                    e.InteractionInformation.DisplayRow, e.InteractionInformation.DisplayColumn, "Descend"), ...
                    Payload=this.contextPayload(e)), ...
                "SortNone", @(~,e)this.invokeCommand("SortNone", ...
                    @()owner.Sort.requestSortByContext( ...
                    e.InteractionInformation.DisplayRow, e.InteractionInformation.DisplayColumn, "None"), ...
                    Payload=this.contextPayload(e)), ...
                "CellSelection", @(~,~)this.invokeCommand("CellSelection", ...
                    @()owner.Selection.requestCellSelection()), ...
                "RowSelection", @(~,~)this.invokeCommand("RowSelection", ...
                    @()owner.Selection.requestRowSelection()), ...
                "ColumnSelection", @(~,~)this.invokeCommand("ColumnSelection", ...
                    @()owner.Selection.requestColumnSelection()), ...
                "ToggleRowFilter", @(~,~)this.invokeCommand("ToggleRowFilter", ...
                    @()this.toggleRowFilter()), ...
                "AutoResizeColumns", @(~,~)this.invokeCommand("AutoResizeColumns", ...
                    @()owner.Column.requestAutoResize()), ...
                "ToggleDragging", @(~,~)this.invokeCommand("ToggleDragging", ...
                    @()this.toggleDragging()), ...
                "ToggleGroupHeaderTooltips", @(~,~)this.invokeCommand("ToggleGroupHeaderTooltips", ...
                    @()this.toggleGroupHeaderTooltips()), ...
                "ToggleTableMetrics", @(~,~)this.invokeCommand("ToggleTableMetrics", ...
                    @()this.toggleTableMetrics()), ...
                "ToggleDisplayOrientation", @(~,~)this.invokeCommand("ToggleDisplayOrientation", ...
                    @()this.toggleDisplayOrientation()));
        end

        function invokeCommand(this, command, action, nvp)
            arguments
                this (1,1) gwidgets.internal.table.ContextMenuController
                command (1,1) string
                action (1,1) function_handle
                nvp.Value = []
                nvp.Payload (1,1) struct = struct()
            end

            payload = nvp.Payload;
            payload.Source = "contextMenu";
            eventData = this.owner().emitTableEvent("CommandInvoked", ...
                gwidgets.table.TableEventData( ...
                Action=command, ...
                Command=command, ...
                Value=nvp.Value, ...
                Payload=payload));
            if eventData.Cancel || eventData.Handled
                return
            end
        ...
        function payload = contextPayload(~, eventData)
            info = eventData.InteractionInformation;
            payload = struct( ...
